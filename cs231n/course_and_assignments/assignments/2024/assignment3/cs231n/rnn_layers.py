...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h_pre = x.dot(Wx) + prev_h.dot(Wh)+ b
    next_h = np.tanh(next_h_pre)
    
    cache = (x, Wx, Wh, b, prev_h, next_h_pre)
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    (x, Wx, Wh, b, prev_h, next_h_pre) = cache
    def dtanh(x):
        return 1 - np.tanh(x)**2
    
    dm = dnext_h * dtanh(next_h_pre)
#    print("dm", dm.shape)
#    print("x", x.shape)
#    print("Wx", Wx.shape)
    dx = dm.dot(Wx.T)
...
#    print("x", x.shape)
    ...
    for idx in range(x.shape[0]):
        xi = x[idx]
        new_h, c  = rnn_step_forward(xi, prev_h, Wx, Wh, b)
        h[idx] = new_h
        prev_h = new_h
        cache_list.append(c)
...
#    print("dh", dh.shape)
    ...
    for idx in range(T -1 , -1, -1):
        if idx != T - 1:
            dnext_h += dh[idx]
        c = cache_list.pop()
        dx[idx], dprev_hi, dWxi, dWhi, dbi = rnn_step_backward(dnext_h, c)
        dnext_h = dprev_hi
        dWx += dWxi
        dWh += dWhi
        db += dbi
