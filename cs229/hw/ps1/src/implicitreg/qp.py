...
#        print(Y)
        for t in range(max_step):
            ...
            def gradient(X, Y, var):
                n = len(X)
                sum = np.zeros(len(X[0]))
                theta_phi_square = element_wise_product(self.theta, self.theta) - element_wise_product(self.phi, self.phi)
#                print("t_p_square")
#                print(theta_phi_square)
                for i in range(n):
                    sum += (X[i].dot(theta_phi_square) - Y[i]) * element_wise_product(var, X[i])
                return sum / n;

            theta_step = eta * gradient(X, Y, self.theta)
            phi_step = eta * -1 * gradient(X,Y,self.phi)
...
#                print(theta_phi_square)
            ...
            theta_step = eta * gradient(X_batch, Y_batch, self.theta)
            phi_step = eta * -1 * gradient(X_batch,Y_batch,self.phi)
